...
export default function PlaceholderPage({ path }: { path: string }) {
  ...
  return <Layout><section className="paper-grid min-h-[58vh] py-16 sm:py-24"><Container><div className="mx-auto max-w-2xl rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card)/.92)] p-7 text-center shadow-[var(--shadow)] sm:p-12"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><Icon size={25} /></span><SectionTitle eyebrow={content.eyebrow} title={content.title} description={content.description} /><Button href="/" variant="secondary" className="mt-8" data-testid={`button-placeholder-home-${path.slice(1)}`}>Return home</Button></div></Container></section></Layout>;
}