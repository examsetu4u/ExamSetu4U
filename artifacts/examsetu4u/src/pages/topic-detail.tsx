      ...
      <Container>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">{contentItems.map((item) => {
          ...
          return <Card key={item.key} className={`flex h-full flex-col p-5 ${available ? 'transition hover:-translate-y-0.5 hover:border-[hsl(var(--accent))] hover:shadow-[var(--shadow-md)]' : 'opacity-75'}`}>
            ...
            <div className="mt-5">{available ? item.key === 'studyMaterial' ? <Button href={`/study-material/${exam.id}/${subject.id}/${topic.id}`} variant="secondary" className="w-full" onClick={() => setTopicProgress(topic.id, Math.max(progress, 20))}>Read material <ArrowRight size={15} /></Button> : <Button type="button" variant="secondary" className="w-full" onClick={() => setTopicProgress(topic.id, Math.max(progress, 50))}>Open sample <ArrowRight size={15} /></Button> : <span className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-[hsl(var(--secondary))] px-4 py-2.5 text-sm font-bold text-[hsl(var(--muted-foreground))]">Content will be added later</span>}</div>
          </Card>;
        })}</div>
