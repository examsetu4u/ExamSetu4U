        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">{contentItems.map((item) => {
          ...
          return <Card key={item.key} className={`flex h-full flex-col p-5 ${available ? 'transition hover:-translate-y-0.5 hover:border-[hsl(var(--accent))] hover:shadow-[var(--shadow-md)]' : 'opacity-75'}`}>
            ...
            <h3 className="mt-5 text-xl font-bold text-[hsl(var(--primary))]">{item.title}</h3><p className="mt-2 flex-1 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{item.description}</p>
             <div className="mt-5">{available ? item.key === 'studyMaterial' ? <Button href={`/study-material/${exam.id}/${subject.id}/${topic.id}`} variant="secondary" className="w-full" onClick={() => setTopicProgress(topic.id, Math.max(progress, 20))}>Read material <ArrowRight size={15} /></Button> : item.key === 'pyq' ? <Button href={`/pyq/${exam.id}/${subject.id}/${topic.id}`} variant="secondary" className="w-full" onClick={() => setTopicProgress(topic.id, Math.max(progress, 50))}>Start practice <ArrowRight size={15} /></Button> : <Button type="button" variant="secondary" className="w-full" onClick={() => setTopicProgress(topic.id, Math.max(progress, 50))}>Open sample <ArrowRight size={15} /></Button> : <span className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-[hsl(var(--secondary))] px-4 py-2.5 text-sm font-bold text-[hsl(var(--muted-foreground))]">Content will be added later</span>}</div>
          </Card>;
        })}</div>
         <div className="mt-10 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 sm:p-6"><div className="flex items-start gap-3"><BookOpen className="mt-0.5 shrink-0 text-[hsl(var(--accent-foreground))]" /><div><h3 className="font-bold text-[hsl(var(--primary))]">Module 3 study reader</h3><p className="mt-1 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Start with focused notes, important exam cues and a quick revision panel. Your completion and bookmarks stay saved on this device.</p></div></div></div>
